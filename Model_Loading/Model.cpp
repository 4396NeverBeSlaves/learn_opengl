...
Model::Model(const string& path):model_matrix(1.0),model_normal_matrix(1.0) {
	load_model(path);
}
...
void Model::draw(Shader& s) {
	...
	s.use();
	s.set_matrix("view", cam.get_view_matrix());
	s.set_matrix("projection", perspective((float)radians(cam.fov), (float)WIDTH / HEIGHT, 0.1f, 100.0f));
	s.set_uniform_3fv("eye_pos", cam.cam_pos);
	s.set_matrix("model", model_matrix);
	s.set_matrix("normal_matrix", model_normal_matrix);
	for (int i = 0; i < meshes.size(); i++) {
		meshes[i]->draw(s);
	}
...
void Model::translate(Shader& s, const vec3& v) {
...
void Model::rotate(Shader& s, float rad, const vec3& v) {
