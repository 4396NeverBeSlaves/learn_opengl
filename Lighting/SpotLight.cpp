...
namespace SpotLight {
	...
	int main() {
		...
		Shader objshader("obj_shader.vert", "spot_light_obj.frag");
		...
		vec3 light_pos = vec3(1.0f, 1.5f, 3.0f);
		vec3 light_dir = cam.cam_dir;
		...
		while (!glfwWindowShouldClose(w)) {
			...
			for (int i = 0; i < 10; i++) {
				...
				objshader.set_uniform_3fv("light.light_pos", vec3(cam.cam_pos.x + 0.5, cam.cam_pos.y - 0.5, cam.cam_pos.z));
				objshader.set_uniform_3fv("light.spot_dir", cam.cam_dir);
			...
			//model = rotate(model, (float)radians(10.0) * current_time, vec3(0, 1, 0));
			model = translate(model, light_pos);

			light_pos = model * vec4(light_pos, 1.0);	//更新旋转后的灯光位置，使objshader能获得新的灯光位置
			...
			lightshader.set_uniform_3fv("light_color", vec3(1.0, 0.0, 0.0));
			glBindVertexArray(lightvao);
			glDrawArrays(GL_TRIANGLES, 0, 36);
